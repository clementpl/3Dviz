...
module.exports = class RendererManager {
    ...
    initTypes() {
        this.font = new THREE.Font(this.loadFont());
        this.types = {
            'text': {
                'create': (options = {}) => {
                    console.log(this.font);
                        var geometry = new THREE.TextGeometry(options.text, {
                            font: this.font,
                            size: 50,
                            height: 10,
                            curveSegments: 12,
                        ...
                        if (options.position)
                            t.position.copy(new THREE.Vector3(options.position.x, options.position.y, options.position.z));
                        return t;
                }
            },
            'cube': {
            ...
            'grid': {
                'create': (options = {}) => {
                    ...
                    if (!options.drawLine || options.drawLine == 'vertical')
                    ...
                    if (!options.drawLine || options.drawLine == 'horizontal')
                    ...
                    grid.position.x = options.position.x || -(xSize * (xLength + 1)) / 2;
                    grid.position.z = options.position.z || (zSize * (zLength - 1)) / 2; // - zSize/2;
                    grid.position.y = options.position.y || -options.yOffset;

                    if (options.rotation) {
                        grid.rotation.z = options.rotation.z || grid.rotation.z;
                        grid.rotation.x = options.rotation.x || grid.rotation.x;
                    }
                    return grid;
                }
            },
    ...
    setCameraPosition(options = {}) {
        if (!options.position)
            position = {};
        this.camera.position.x = options.position.x || this.camera.position.x;
        this.camera.position.y = options.position.y || this.camera.position.y;
        this.camera.position.z = options.position.z || this.camera.position.z;
    }
    ...
    addMouseControls() {
        ...
        //        this.cameraControls.target.set(0, 0, 500);
